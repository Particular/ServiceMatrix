...
{
    /// <summary>
    /// A custom command design-time validation rule for the <see cref="ShowSubscriberPicker"/> command.
    /// </summary>
    [CommandValidationRule(typeof(ShowSubscriberPicker))]
    [CLSCompliant(false)]
    public class ShowSubscriberPickerValidation : ICommandValidationRule
    {
        private static readonly ITraceSource tracer = Tracer.GetSourceFor<ShowSubscriberPickerValidation>();
        ...
        public void Validate(Microsoft.VisualStudio.Modeling.Validation.ValidationContext context, IAutomationSettingsSchema settingsElement, ICommandSettings settings)
        {
            try
            {
                ...
                //var propValue1 = settings.GetOrCreatePropertyValue(Reflector<ShowSubscriberPicker>.GetPropertyName(s => s.AProperty), String.Empty);
            ...
            catch (Exception ex)
            {
                ...
                tracer.TraceError(
                    "Validation rule ShowSubscriberPickerValidation unexpectedly failed, error was: '{0}'", ex.Message);
                throw;
            }
        }
    }
