        ...
        private void SetPropagationHandlers()
        {
            ...
            TransportChanged += (s, e) =>
            {
                ...
                else if (Transport == TransportType.RabbitMQ.ToString())
                {
                    ...
                    Design.Endpoints.GetAll().ForEach(x => x.Project.InstallNuGetPackage(VsPackageInstallerServices, VsPackageInstaller, StatusBar, "NServiceBus.RabbitMQ"));
                }
                else if (Transport == TransportType.SqlServer.ToString())
                {
                    ...
                    Design.Endpoints.GetAll().ForEach(x => x.Project.InstallNuGetPackage(VsPackageInstallerServices, VsPackageInstaller, StatusBar, "NServiceBus.SqlServer"));
                }
                else if (Transport == TransportType.AzureQueues.ToString())
                {
                    TransportConnectionString = @"UseDevelopmentStorage=True;";
                    Design.Endpoints.GetAll().ForEach(x => x.Project.InstallNuGetPackage(VsPackageInstallerServices, VsPackageInstaller, StatusBar, "NServiceBus.Azure.Transports.WindowsAzureStorageQueues"));
                }
                else if (Transport == TransportType.AzureServiceBus.ToString())
                {
                    TransportConnectionString = @"UseDevelopmentStorage=True;";
                    Design.Endpoints.GetAll().ForEach(x => x.Project.InstallNuGetPackage(VsPackageInstallerServices, VsPackageInstaller, StatusBar, "NServiceBus.Azure.Transports.WindowsAzureServiceBus"));
                }
            };
        }
